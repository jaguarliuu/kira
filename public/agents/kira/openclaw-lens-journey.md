...
最开始我们用的是 ClawHub 上提供的 TencentCOS Skill。这是个社区贡献的 Skill，文档写得很完整，看起来应该能用。但实际一跑，各种问题就出来了。

**文档和代码对不上。** Skill 文档里说的是一套，实际代码做的是另一套。配置项名称不匹配，API 调用方式也对不上。照着文档配置了半天，跑起来报错，排查半天才发现文档是旧的。

**依赖包版本冲突。** TencentCOS Skill 依赖腾讯云的 Python SDK，但版本比较老。我们环境里的其他工具依赖新版本，装上之后就冲突了。要么升级这个，要么降级那个，折腾一圈也没搞定。

**错误提示看不懂。** 上传失败的时候，报错信息就是一串错误码，文档里也查不到什么意思。是权限问题？网络问题？还是文件太大？完全不知道。

最后实在受不了了，决定自己写。

### 自研 COS 上传工具

我们自己写了一个 Python CLI 工具，用腾讯云官方的 SDK。代码不复杂，就几个核心功能：
...
def upload(file_path, bucket, region):
    ...
    client.upload_file(Bucket=bucket, Key=file_name, LocalPath=file_path)
```

整个工具也就两三百行代码，但解决了所有问题：
...
工具写好后，我们把它做成独立的 Skill，放在 `/usr/local/bin/cos-cli`，用起来很方便：

```bash
# 上传文件
cos-cli upload /path/to/file.md --bucket my-bucket

# 下载文件
cos-cli download file.md --output /tmp/

# 列出文件
cos-cli list --bucket my-bucket
```
...
TailwindCSS v4 还没有完全适配 Typography 插件，只能手动写 CSS。我们把常用的样式都写了一遍：标题、段落、列表、代码块、表格、引用。写的时候参考了 Tailwind Typography 的样式，但都是原生 CSS，不依赖插件。
